...
function renderPagination(totalPages) {
  ...
  const $prev = $('<button>').addClass('page-btn').text('←').prop('disabled', currentPage === 0).on('click', () => loadPage(currentPage - 1));
  $pag.append($prev);
  getPageNumbers(currentPage, totalPages).forEach(i => {
    ...
    const $btn = $('<button>').addClass('page-btn' + (i === currentPage ? ' active' : '')).text(i + 1).on('click', () => loadPage(i));
    $pag.append($btn);
  });
  const $info = $('<span>').addClass('page-info').text(`${currentPage * LIMIT + 1}–${Math.min((currentPage + 1) * LIMIT, totalProducts)} of ${totalProducts}`);
  $pag.append($info);
  const $next = $('<button>').addClass('page-btn').text('→').prop('disabled', currentPage >= totalPages - 1).on('click', () => loadPage(currentPage + 1));
  $pag.append($next);
}
